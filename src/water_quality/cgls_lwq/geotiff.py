...
def parse_geotiff_url(geotiff_url: str) -> tuple[str]:
    ...
    acronym, date_str, area, sensor, version, tile_index_str, *subdataset_variable = parts
...
def get_dataset_tile_id(geotiff_url: str) -> str:
    ...
    dataset_tile_id = (
        f"{filename_prefix}_{acronym}_{date_str}_{area}_{sensor}_{version}_{tile_index_str}"
    )
